...
JET_BEGIN_TEST_F(PciSphSolver3, SteadyState) {
    ...
    BoundingBox3D initialBound(Point3D(), Point3D(1, 0.5, 1));
    initialBound.expand(-targetSpacing);

    auto emitter = std::make_shared<VolumeParticleEmitter3>(
            std::make_shared<SurfaceToImplicit3>(std::make_shared<Sphere3>(Point3D(), 10.0)), initialBound,
            targetSpacing, Vector3D());
    emitter->setJitter(0.0);
    solver.setEmitter(emitter);

    Box3Ptr box = std::make_shared<Box3>(Point3D(), Point3D(1, 1, 1));
...
JET_BEGIN_TEST_F(PciSphSolver3, WaterDrop) {
    ...
    BoundingBox3D domain(Point3D(), Point3D(1, 2, 0.5));
    ...
    surfaceSet->addExplicitSurface(std::make_shared<Plane3>(Vector3D(0, 1, 0), Point3D(0, 0.25 * domain.height(), 0)));
