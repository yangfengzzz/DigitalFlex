...
public:
    double sample(const Point3D& x) const override { return std::sin(x.x) * std::sin(x.y) * std::sin(x.z); }

    Vector3D gradient(const Point3D& x) const override {
        return Vector3D(std::cos(x.x) * std::sin(x.y) * std::sin(x.z), std::sin(x.x) * std::cos(x.y) * std::sin(x.z),
                        std::sin(x.x) * std::sin(x.y) * std::cos(x.z));
    }

    double laplacian(const Point3D& x) const override {
        return -std::sin(x.x) * std::sin(x.y) * std::sin(x.z) - std::sin(x.x) * std::sin(x.y) * std::sin(x.z) -
...
JET_BEGIN_TEST_F(ScalarField3, Sample) {
    ...
    for (int j = 0; j < 50; ++j) {
        for (int i = 0; i < 50; ++i) {
            Point3D x(0.04 * kPiD * i, 0.04 * kPiD * j, kHalfPiD);
...
JET_BEGIN_TEST_F(ScalarField3, Gradient) {
    ...
    for (int j = 0; j < 20; ++j) {
        for (int i = 0; i < 20; ++i) {
            Point3D x(0.1 * kPiD * i, 0.1 * kPiD * j, kHalfPiD);
...
public:
    Vector3D sample(const Point3D& x) const override {
        return Vector3D(std::sin(x.x) * std::sin(x.y), std::sin(x.y) * std::sin(x.z), std::sin(x.z) * std::sin(x.x));
    }

    double divergence(const Point3D& x) const override {
        return std::cos(x.x) * std::sin(x.y) + std::cos(x.y) * std::sin(x.z) + std::cos(x.z) * std::sin(x.x);
    }

    Vector3D curl(const Point3D& x) const override {
        return Vector3D(-std::sin(x.y) * std::cos(x.z), -std::sin(x.z) * std::cos(x.x), -std::sin(x.x) * std::cos(x.y));
    }
};

class MyCustomVectorField3_2 final : public VectorField3 {
public:
    Vector3D sample(const Point3D& x) const override { return Vector3D(-x.y, x.x, 0.0); }
};
...
JET_BEGIN_TEST_F(VectorField3, Curl) {
    ...
    for (int j = 0; j < 20; ++j) {
        for (int i = 0; i < 20; ++i) {
            Point3D x(0.1 * kPiD * i, 0.1 * kPiD * j, 0.5 * kHalfPiD);
...
JET_BEGIN_TEST_F(VectorField3, Sample2) {
    ...
    for (int j = 0; j < 20; ++j) {
        for (int i = 0; i < 20; ++i) {
            Point3D x(0.05 * i - 0.5, 0.05 * j - 0.5, 0.5);
            dataU(i, j) = field.sample(x).x;
            dataV(i, j) = field.sample(x).y;
        }
    }
