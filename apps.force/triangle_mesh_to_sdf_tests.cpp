...
JET_BEGIN_TEST_F(TriangleMeshToSdf, Bunny) {
    ...
    std::ifstream file("./assets/Models/bunny.obj");
    if (file) {
    ...
    box.lower_corner -= 0.2 * scale;
    box.upper_corner += 0.2 * scale;

    VertexCenteredScalarGrid3 grid(100, 100, 100, box.width() / 100, box.height() / 100, box.depth() / 100,
                                   box.lower_corner.x, box.lower_corner.y, box.lower_corner.z);
...
JET_BEGIN_TEST_F(TriangleMeshToSdf, Dragon) {
    ...
    std::ifstream file("./assets/Models/dragon.obj");
    if (file) {
        triMesh.readObj(&file);
        file.close();
    }
