...
JET_BEGIN_TEST_F(MarchingCubes, FourCubes) {
    ...
    grid.fill([](const Point3D& x) { return x.y - 0.5; });
...
JET_BEGIN_TEST_F(MarchingCubes, Sphere) {
    ...
    grid.fill([](const Point3D& x) { return x.distanceTo({8.0, 8.0, 8.0}) - 3.0; });

    marchingCubes(grid.constDataAccessor(), grid.gridSpacing(), grid.origin(), &triMesh, 0, kDirectionAll);

    saveTriangleMeshData(triMesh, "sphere.obj");

    grid.fill([](const Point3D& x) { return x.distanceTo({0.0, 4.0, 3.0}) - 6.0; });
    ...
    grid.fill([](const Point3D& x) { return x.distanceTo({11.0, 14.0, 12.0}) - 6.0; });
