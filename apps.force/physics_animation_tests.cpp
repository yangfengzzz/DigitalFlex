...
protected:
    void onAdvanceTimeStep(double timeIntervalInSeconds) override {
        ...
        for (size_t i = 0; i < numberOfEdges; ++i) {
            ...
            Point3D pos0 = positions[pointIndex0];
            Point3D pos1 = positions[pointIndex1];
        ...
        for (size_t i = 0; i < numberOfPoints; ++i) {
            ...
            Point3D newPosition = positions[i] + timeIntervalInSeconds * newVelocity;
...
JET_BEGIN_TEST_F(PhysicsAnimation, SimpleMassSpringAnimation) {
    ...
    anim.constraints.push_back(SimpleMassSpringAnimation::Constraint{0, Point3D(), Vector3D()});
