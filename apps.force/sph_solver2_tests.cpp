...
JET_BEGIN_TEST_F(SphSolver2, SteadyState) {
    ...
    BoundingBox2D initialBound(Point2D(), Point2D(1, 0.5));
    initialBound.expand(-targetSpacing);

    auto emitter = std::make_shared<VolumeParticleEmitter2>(
            std::make_shared<SurfaceToImplicit2>(std::make_shared<Sphere2>(Point2D(), 10.0)), initialBound,
            particles->targetSpacing(), Vector2D());
    emitter->setJitter(0.0);
    solver.setEmitter(emitter);

    Box2Ptr box = std::make_shared<Box2>(Point2D(), Point2D(1, 1));
...
JET_BEGIN_TEST_F(SphSolver2, WaterDrop) {
    ...
    surfaceSet->addExplicitSurface(std::make_shared<Plane2>(Vector2D(0, 1), Point2D(0, 0.25 * domain.height())));
