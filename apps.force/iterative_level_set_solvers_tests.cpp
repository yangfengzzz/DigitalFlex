...
JET_BEGIN_TEST_F(LevelSetSolver2, Reinitialize) {
    ...
    data0.fill([&](const Point2D& pt) { return pt.distanceTo(Point2D(0.5, 0.75)) - 0.15; });

    auto flowFunc = [&](const Point2D& pt) {
        Vector2D ret;
    ...
    dataU.fill([&](const Point2D& pt) { return flowFunc(pt).x; });
    dataV.fill([&](const Point2D& pt) { return flowFunc(pt).y; });
...
JET_BEGIN_TEST_F(LevelSetSolver2, NoReinitialize) {
    ...
    CustomVectorField2 flow([&](const Point2D& pt) {
        Vector2D ret;
        ret.x = 2.0 * square(std::sin(kPiD * pt.x)) * std::sin(kPiD * pt.y) * std::cos(kPiD * pt.y);
        ret.y = -2.0 * square(std::sin(kPiD * pt.y)) * std::sin(kPiD * pt.x) * std::cos(kPiD * pt.x);
        return ret;
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver2, ReinitializeSmall) {
    ...
    sdf.fill([](const Point2D& x) { return 1.0; });
    ...
    sdf.fill([](const Point2D& x) { return (x - Point2D(20, 20)).length() - 8.0; });
    ...
    sdf.fill([](const Point2D& x) {
        double r = (x - Point2D(20, 20)).length() - 8.0;
        return 2.0 * r;
    });
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver2, Reinitialize) {
    ...
    // Starting from SDF field
    sdf.fill([](const Point2D& x) { return (x - Point2D(80, 80)).length() - 32.0; });
    saveData(sdf.constDataAccessor(), "sdf0_#grid2,iso.npy");
    ...
    // Starting from scaled SDF field
    sdf.fill([](const Point2D& x) {
        double r = (x - Point2D(80, 80)).length() - 32.0;
        return 2.0 * r;
    });
    saveData(sdf.constDataAccessor(), "scaled0_#grid2,iso.npy");
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver2, Extrapolate) {
    ...
    sdf.fill([&](const Point2D& x) { return (x - Point2D(0.75, 0.5)).length() - 0.3; });

    input.fill([&](const Point2D& x) {
        if ((x - Point2D(0.75, 0.5)).length() <= 0.3) {
            double p = 10.0 * kPiD;
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver3, ReinitializeSmall) {
    ...
    sdf.fill([](const Point3D& x) { return (x - Point3D(20, 20, 20)).length() - 8.0; });
...
JET_BEGIN_TEST_F(UpwindLevelSetSolver3, ExtrapolateSmall) {
    ...
    sdf.fill([](const Point3D& x) { return (x - Point3D(20, 20, 20)).length() - 8.0; });
    field.fill([&](const Point3D& x) {
        if ((x - Point3D(20, 20, 20)).length() <= 8.0) {
