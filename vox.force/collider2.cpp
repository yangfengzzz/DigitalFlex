...
Collider2::Collider2() = default;

Collider2::~Collider2() = default;

void Collider2::resolveCollision(double radius,
    ...
    if (isPenetrating(colliderPoint, *newPosition, radius)) {
        ...
        Point2D targetPoint = colliderPoint.point + radius * targetNormal;
        ...
        if (normalDotRelativeVel < 0.0) {
            ...
            if (relativeVelT.lengthSquared() > 0.0) {
                double frictionScale =
                        std::max(1.0 - _frictionCoeffient * deltaRelativeVelN.length() / relativeVelT.length(), 0.0);
...
bool Collider2::isPenetrating(const ColliderQueryResult& colliderPoint, const Point2D& position, double radius) {
