...
void SphSolver3::accumulatePressureForce(const ConstArrayAccessor1<Point3D>& positions,
                                         const ConstArrayAccessor1<double>& densities,
                                         const ConstArrayAccessor1<double>& pressures,
                                         ArrayAccessor1<Vector3D> pressureForces) const {
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
...
SphSolver3::Builder SphSolver3::builder() { return {}; }

SphSolver3 SphSolver3::Builder::build() const { return {_targetDensity, _targetSpacing, _relativeKernelRadius}; }

SphSolver3Ptr SphSolver3::Builder::makeShared() const {
    return {new SphSolver3(_targetDensity, _targetSpacing, _relativeKernelRadius), [](SphSolver3* obj) { delete obj; }};
}
