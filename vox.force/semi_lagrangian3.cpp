...
SemiLagrangian3::SemiLagrangian3() = default;

SemiLagrangian3::~SemiLagrangian3() = default;

void SemiLagrangian3::advect(const ScalarGrid3& input,
    ...
    output->parallelForEachDataPointIndex([&](size_t i, size_t j, size_t k) {
        if (boundarySdf.sample(inputDataPos(i, j, k)) > 0.0) {
            Point3D pt = backTrace(flow, dt, h, outputDataPos(i, j, k), boundarySdf);
...
void SemiLagrangian3::advect(const FaceCenteredGrid3& input,
    ...
    output->parallelForEachUIndex([&](size_t i, size_t j, size_t k) {
        if (boundarySdf.sample(uSourceDataPos(i, j, k)) > 0.0) {
            Point3D pt = backTrace(flow, dt, h, uTargetDataPos(i, j, k), boundarySdf);
    ...
    output->parallelForEachVIndex([&](size_t i, size_t j, size_t k) {
        if (boundarySdf.sample(vSourceDataPos(i, j, k)) > 0.0) {
            Point3D pt = backTrace(flow, dt, h, vTargetDataPos(i, j, k), boundarySdf);
    ...
    output->parallelForEachWIndex([&](size_t i, size_t j, size_t k) {
        if (boundarySdf.sample(wSourceDataPos(i, j, k)) > 0.0) {
            Point3D pt = backTrace(flow, dt, h, wTargetDataPos(i, j, k), boundarySdf);
            wTargetDataAcc(i, j, k) = inputSamplerFunc(pt).z;
        }
    });
}

Point3D SemiLagrangian3::backTrace(
        const VectorField3& flow, double dt, double h, const Point3D& startPt, const ScalarField3& boundarySdf) {
    ...
    while (remainingT > kEpsilonD) {
        ...
        if (phi0 * phi1 < 0.0) {
            ...
            pt1 = Point3D(w * pt0.x + (1.0 - w) * pt1.x, w * pt0.y + (1.0 - w) * pt1.y, w * pt0.z + (1.0 - w) * pt1.z);
            break;
        }
...
std::function<double(const Point3D&)> SemiLagrangian3::getScalarSamplerFunc(const ScalarGrid3& input) const {
    return input.sampler();
}

std::function<Vector3D(const Point3D&)> SemiLagrangian3::getVectorSamplerFunc(
        const CollocatedVectorGrid3& input) const {
    return input.sampler();
}

std::function<Vector3D(const Point3D&)> SemiLagrangian3::getVectorSamplerFunc(const FaceCenteredGrid3& input) const {
    return input.sampler();
}
