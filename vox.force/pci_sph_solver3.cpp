...
#include "vox.force/pci_sph_solver3.h"
...
PciSphSolver3::~PciSphSolver3() = default;
...
void PciSphSolver3::accumulatePressureForce(double timeIntervalInSeconds) {
    ...
    for (unsigned int k = 0; k < _maxNumberOfIterations; ++k) {
        ...
        // Resolve collisions
        resolveCollision(_tempPositions.accessor(), _tempVelocities.accessor());
    ...
    LOGI("Number of PCI iterations: {}", maxNumIter)
    LOGI("Max density error after PCI iteration: {}", maxDensityError)
    if (std::fabs(densityErrorRatio) > _maxDensityErrorRatio) {
        LOGW("Max density error ratio is greater than the threshold!")
        LOGW("Ratio: {} Threshold: {}", densityErrorRatio, _maxDensityErrorRatio)
    }
...
double PciSphSolver3::computeDelta(double timeStepInSeconds) {
    ...
    for (auto& i : points) {
        const Vector3D point = Vector3D(i.x, i.y, i.z);
...
PciSphSolver3::Builder PciSphSolver3::builder() { return {}; }

PciSphSolver3 PciSphSolver3::Builder::build() const { return {_targetDensity, _targetSpacing, _relativeKernelRadius}; }

PciSphSolver3Ptr PciSphSolver3::Builder::makeShared() const {
    return {new PciSphSolver3(_targetDensity, _targetSpacing, _relativeKernelRadius),
