...
#include "vox.force/vorticity_micropolar.h"
...
MicropolarModel_Bender2017::MicropolarModel_Bender2017(FluidModel *model) : VorticityBase(model) {
    ...
    m_angularAcceleration.resize(model->numParticles(), Vector3D());
...
void MicropolarModel_Bender2017::initParameters() {
    ...
    auto *rparam = static_cast<utility::DoubleParameter *>(getParameter(VISCOSITY_OMEGA));
    ...
    rparam = static_cast<utility::DoubleParameter *>(getParameter(INERTIA_INVERSE));
    rparam->setMinValue(0.0);
}
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            Vector3D &angAcceli = m_angularAcceleration[i];
            angAcceli.setZero();
            ...
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                ...
                forall_density_maps(
                        Vector3D vj; bm_neighbor->getPointVelocity(xi, vj);
                        const Vector3D &omegaij = omegai;  // ToDo: omega_j

                        // XSPH for angular velocity field
                        // angAcceli -= invDt * m_inertiaInverse * zeta * (density0 / density_i) * omegaij * rho;

                        // difference curl
                        const Vector3D a = nu_t * density0 / density_i * (omegaij.cross(gradRho));
                        ai += a;
                ...
                forall_volume_maps(Vector3D vj; bm_neighbor->getPointVelocity(xj, vj);
                                   const Vector3D omegaij = omegai;  // ToDo: omega_j

                                   const Vector3D xij = xi - xj; const Vector3D gradW = sim->gradW(xij);
                                   ...
                                   // difference curl
                                   const Vector3D a = (nu_t * 1.0 / density_i * density0 * Vj) * (omegaij.cross(gradW));
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            forall_fluid_neighbors_in_same_phase(
                    const Vector3D &vj = m_model->getVelocity(neighborIndex);
                    const Vector3D &omegaj = m_omega[neighborIndex];

                    // Viscosity
                    const Real density_j = m_model->getDensity(neighborIndex);
                    const Vector3D xij = xi - xj; const Vector3D omegaij = omegai - omegaj;
                    const Vector3D gradW = sim->gradW(xij);
                ...
                forall_boundary_neighbors(
                        const Vector3D &vj = bm_neighbor->getVelocity(neighborIndex);
                        const Vector3D &omegaj = Vector3D();  // m_omega[neighborIndex];

                        // Viscosity
                        const Vector3D xij = xi - xj;
                        const Vector3D omegaij = omegai - omegaj; const Vector3D gradW = sim->gradW(xij);
                        ...
                        // difference curl
                        const Vector3D a = nu_t * 1.0 / density_i * density0 * bm_neighbor->getVolume(neighborIndex) *
                                           (omegaij.cross(gradW));
                        ai += a;
                        angAcceli += nu_t * 1.0 / density_i * m_inertiaInverse *
                ...
                forall_density_maps(Vector3D vj; bm_neighbor->getPointVelocity(xi, vj);
                                    const Vector3D omegaij = omegai;  // ToDo: omega_j
                ...
                forall_volume_maps(Vector3D vj; bm_neighbor->getPointVelocity(xj, vj);
                                   ...
                                   // difference curl
                                   const Vector3D a = nu_t * 1.0 / density_i * density0 * Vj * (omegaij.cross(gradW));
                                   ai += a; angAcceli += nu_t * 1.0 / density_i * m_inertiaInverse *
                                                         (density0 * Vj * (vi - vj).cross(gradW));
