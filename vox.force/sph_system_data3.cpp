...
double SphSystemData3::sumOfKernelNearby(const Point3D& origin) const {
    double sum = 0.0;
    SphStdKernel3 kernel(_kernelRadius);
    neighborSearcher()->forEachNearbyPoint(origin, _kernelRadius, [&](size_t, const Point3D& neighborPosition) {
...
double SphSystemData3::interpolate(const Point3D& origin, const ConstArrayAccessor1<double>& values) const {
    double sum = 0.0;
    auto d = densities();
    SphStdKernel3 kernel(_kernelRadius);
    const double m = mass();

    neighborSearcher()->forEachNearbyPoint(origin, _kernelRadius, [&](size_t i, const Point3D& neighborPosition) {
        double dist = origin.distanceTo(neighborPosition);
...
Vector3D SphSystemData3::interpolate(const Point3D& origin, const ConstArrayAccessor1<Vector3D>& values) const {
    Vector3D sum;
    auto d = densities();
    SphStdKernel3 kernel(_kernelRadius);
    const double m = mass();
...
Vector3D SphSystemData3::gradientAt(size_t i, const ConstArrayAccessor1<double>& values) const {
    ...
    const Point3D& origin = p[i];
    SphSpikyKernel3 kernel(_kernelRadius);
    const double m = mass();

    for (size_t j : neighbors) {
        const Point3D& neighborPosition = p[j];
        double dist = origin.distanceTo(neighborPosition);
        if (dist > 0.0) {
...
void SphSystemData3::computeMass() {
    ...
    BoundingBox3D sampleBound(Point3D(-1.5 * _kernelRadius, -1.5 * _kernelRadius, -1.5 * _kernelRadius),
                              Point3D(1.5 * _kernelRadius, 1.5 * _kernelRadius, 1.5 * _kernelRadius));
