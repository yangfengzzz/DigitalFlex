...
protected:
    ...
    //!
    [[nodiscard]] virtual std::function<double(const Point2D&)> getScalarSamplerFunc(const ScalarGrid2& input) const;
    ...
    //! interpolation for semi-Lagrangian process.
    //!
    [[nodiscard]] virtual std::function<Vector2D(const Point2D&)> getVectorSamplerFunc(
...
private:
    static Point2D backTrace(
            const VectorField2& flow, double dt, double h, const Point2D& pt0, const ScalarField2& boundarySdf);
};
