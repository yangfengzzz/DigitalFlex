...
public:
    ...
    [[nodiscard]] virtual Vector2D velocityAt(const Point2D& point) const = 0;
    ...
    //!
    void resolveCollision(double radius, double restitutionCoefficient, Point2D* position, Vector2D* velocity);
    ...
    [[nodiscard]] const Surface2Ptr& surface() const;
...
protected:
    ...
    void getClosestPoint(const Surface2Ptr& surface, const Point2D& queryPoint, ColliderQueryResult* result) const;

    //! Returns true if given point is in the opposite side of the surface.
    bool isPenetrating(const ColliderQueryResult& colliderPoint, const Point2D& position, double radius);
