...
#include "vox.force/iterative_level_set_solver2.h"
...
IterativeLevelSetSolver2::IterativeLevelSetSolver2() = default;
...
void IterativeLevelSetSolver2::extrapolate(const ConstArrayAccessor2<double>& input,
    ...
    for (unsigned int n = 0; n < numberOfIterations; ++n) {
        parallelFor(kZeroSize, size.x, kZeroSize, size.y, [&](size_t i, size_t j) {
            if (sdf(i, j) >= 0) {
                ...
                tempAcc(i, j) =
                        outputAcc(i, j) - dtau * (std::max(grad.x, 0.0) * dx[0] + std::min(grad.x, 0.0) * dx[1] +
...
double IterativeLevelSetSolver2::pseudoTimeStep(const ConstArrayAccessor2<double>& sdf,
