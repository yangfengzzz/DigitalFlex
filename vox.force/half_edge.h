...
public:
    ...
    [[nodiscard]] Halfedge next() const { return {face(), static_cast<unsigned char>((edge() + 1) % 3)}; }

    [[nodiscard]] Halfedge previous() const { return {face(), static_cast<unsigned char>((edge() + 2) % 3)}; }
