...
class TriangleMesh {
public:
    TriangleMesh(std::vector<Vector3D> const &vertices, std::vector<std::array<unsigned int, 3>> const &faces);
    ...
    [[nodiscard]] Vector3D const &vertex(unsigned int i) const { return m_vertices[i]; }
    Vector3D &vertex(unsigned int i) { return m_vertices[i]; }
    ...
    // Data getters.
    [[nodiscard]] std::vector<Vector3D> const &vertex_data() const { return m_vertices; }
    std::vector<Vector3D> &vertex_data() { return m_vertices; }
    ...
    [[nodiscard]] Vector3D computeFaceNormal(unsigned int f) const;
