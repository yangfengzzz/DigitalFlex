...
inline double solveQuadNearBoundary(const Array3<char>& markers,
                                    const ConstArrayAccessor3<double>& output,
...
FmmLevelSetSolver3::FmmLevelSetSolver3() = default;
...
void FmmLevelSetSolver3::reinitialize(const ScalarGrid3& inputSdf, double maxDistance, ScalarGrid3* outputSdf) {
    ...
    markers.forEachIndex([&](size_t i, size_t j, size_t k) {
        if (isInsideSdf(output(i, j, k)) &&
            ...
            output(i, j, k) = solveQuadNearBoundary(markers, inputSdf.constDataAccessor(), gridSpacing,
