...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numBoundaryParticles; i++) {
            ...
            for (unsigned int pid = nFluids; pid < sim->numberOfPointSets(); pid++) {
                auto *bm_neighbor = static_cast<BoundaryModel_Akinci2012 *>(sim->getBoundaryModelFromPointSet(pid));
...
#endif
        m_forcePerThread.resize(maxThreads, Vector3D());
        m_torquePerThread.resize(maxThreads, Vector3D());
    }
