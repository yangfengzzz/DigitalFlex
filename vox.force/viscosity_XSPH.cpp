...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++) {
            ...
            // Fluid
            //////////////////////////////////////////////////////////////////////////
            forall_fluid_neighbors(const Vector3D &vj = fm_neighbor->getVelocity(neighborIndex);
            ...
            if (m_boundaryViscosity != 0.0) {
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
                    forall_boundary_neighbors(const Vector3D &vj = bm_neighbor->getVelocity(neighborIndex);
                                              const Vector3D a =
                    ...
                    forall_volume_maps(Vector3D vj; bm_neighbor->getPointVelocity(xj, vj);
                                       const Vector3D a = -invH * m_boundaryViscosity * (density0 * Vj / density_i) *
                                                          (vi - vj) * sim->W(xi - xj);
                                       ai += a; bm_neighbor->addForce(xj, -m_model->getMass(i) * a););
                }
            }
