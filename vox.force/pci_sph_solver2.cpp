...
PciSphSolver2::~PciSphSolver2() = default;
...
double PciSphSolver2::computeDelta(double timeStepInSeconds) {
    ...
    for (auto& i : points) {
        const Vector2D& point = Vector2D(i.x, i.y);
...
PciSphSolver2::Builder PciSphSolver2::builder() { return {}; }

PciSphSolver2 PciSphSolver2::Builder::build() const { return {_targetDensity, _targetSpacing, _relativeKernelRadius}; }

PciSphSolver2Ptr PciSphSolver2::Builder::makeShared() const {
    return {new PciSphSolver2(_targetDensity, _targetSpacing, _relativeKernelRadius),
