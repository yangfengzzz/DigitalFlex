...
#include "vox.force/sph_solver2.h"
...
#include "vox.force/physics_helpers.h"
...
void SphSolver2::onBeginAdvanceTimeStep(double timeStepInSeconds) {
    ...
    LOGI("Building neighbor lists and updating densities took {} seconds", timer.Elapsed())
}
...
void SphSolver2::onEndAdvanceTimeStep(double timeStepInSeconds) {
    ...
    LOGI("Max density: {} Max density / target density ratio: {}", maxDensity, maxDensity / particles->targetDensity())
}
...
void SphSolver2::accumulatePressureForce(const ConstArrayAccessor1<Point2D>& positions,
                                         const ConstArrayAccessor1<double>& densities,
                                         const ConstArrayAccessor1<double>& pressures,
                                         ArrayAccessor1<Vector2D> pressureForces) const {
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
...
SphSolver2::Builder SphSolver2::builder() { return {}; }

SphSolver2 SphSolver2::Builder::build() const { return {_targetDensity, _targetSpacing, _relativeKernelRadius}; }

SphSolver2Ptr SphSolver2::Builder::makeShared() const {
    return {new SphSolver2(_targetDensity, _targetSpacing, _relativeKernelRadius), [](SphSolver2* obj) { delete obj; }};
}
