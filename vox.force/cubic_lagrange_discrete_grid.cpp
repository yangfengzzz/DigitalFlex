#include "vox.force/cubic_lagrange_discrete_grid.h"
...
Matrix<double, 32, 1> shape_function(Point3D const &xi, Matrix<double, 32, 3> *gradient = nullptr) {
...
Matrix<double, 32, 1> shape_function_(Point3D const &xi, Matrix<double, 32, 3> *gradient = nullptr) {
    auto res = Matrix<double, 32, 1>{};
...
Point3D CubicLagrangeDiscreteGrid::indexToNodePosition(unsigned int l) const {
    ...
    Vector3D ijk;
    if (l < nv) {
        ijk.z = l / ((m_resolution.y + 1) * (m_resolution.x + 1));
        auto temp = l % ((m_resolution.y + 1) * (m_resolution.x + 1));
        ijk.y = temp / (m_resolution.x + 1);
        ijk.x = temp % (m_resolution.x + 1);

        x = m_domain.lower_corner + m_cell_size * ijk;
    } else if (l < nv + 2 * ne_x) {
        l -= nv;
        auto e_ind = l / 2;
        ijk.z = e_ind / ((m_resolution.y + 1) * m_resolution.x);
        auto temp = e_ind % ((m_resolution.y + 1) * m_resolution.x);
        ijk.y = temp / m_resolution.x;
        ijk.x = temp % m_resolution.x;

        x = m_domain.lower_corner + m_cell_size * ijk;
        x.x += (1.0 + static_cast<double>(l % 2)) / 3.0 * m_cell_size.x;
    } else if (l < nv + 2 * (ne_x + ne_y)) {
        l -= (nv + 2 * ne_x);
        auto e_ind = l / 2;
        ijk.x = e_ind / ((m_resolution.z + 1) * m_resolution.y);
        auto temp = e_ind % ((m_resolution.z + 1) * m_resolution.y);
        ijk.z = temp / m_resolution.y;
        ijk.y = temp % m_resolution.y;

        x = m_domain.lower_corner + m_cell_size * ijk;
        x.y += (1.0 + static_cast<double>(l % 2)) / 3.0 * m_cell_size.y;
    } else {
        l -= (nv + 2 * (ne_x + ne_y));
        auto e_ind = l / 2;
        ijk.y = e_ind / ((m_resolution.x + 1) * m_resolution.z);
        auto temp = e_ind % ((m_resolution.x + 1) * m_resolution.z);
        ijk.x = temp / m_resolution.z;
        ijk.z = temp % m_resolution.z;

        x = m_domain.lower_corner + m_cell_size * ijk;
        x.z += (1.0 + static_cast<double>(l % 2)) / 3.0 * m_cell_size.z;
    }
...
#pragma omp for schedule(static) nowait
    ...
    for (auto l = 0u; l < m_n_cells; ++l) {
        auto k = l / (m_resolution.y * m_resolution.x);
        auto temp = l % (m_resolution.y * m_resolution.x);
        auto j = temp / m_resolution.x;
        auto i = temp % m_resolution.x;

        auto nx = m_resolution.x;
        auto ny = m_resolution.y;
        auto nz = m_resolution.z;
...
bool CubicLagrangeDiscreteGrid::determineShapeFunctions(unsigned int field_id,
                                                        ...
                                                        Matrix<double, 32, 3> *dN) const {
    if (!m_domain.contains(x)) return false;

    auto mi = (x - m_domain.lower_corner) * m_inv_cell_size;
    ...
    auto i = multiToSingleIndex({{mi.x, mi.y, mi.z}});
...
void CubicLagrangeDiscreteGrid::forEachCell(
        unsigned int field_id, std::function<void(unsigned int, BoundingBox3D const &, unsigned int)> const &cb) const {
    auto n = m_resolution.x * m_resolution.y * m_resolution.z;
    for (auto i = 0u; i < n; ++i) {
