...
SphSystemData2::~SphSystemData2() = default;
...
double SphSystemData2::sumOfKernelNearby(const Point2D& origin) const {
    double sum = 0.0;
    SphStdKernel2 kernel(_kernelRadius);
    neighborSearcher()->forEachNearbyPoint(origin, _kernelRadius, [&](size_t, const Point2D& neighborPosition) {
...
double SphSystemData2::interpolate(const Point2D& origin, const ConstArrayAccessor1<double>& values) const {
    double sum = 0.0;
    auto d = densities();
    SphStdKernel2 kernel(_kernelRadius);
    const double m = mass();

    neighborSearcher()->forEachNearbyPoint(origin, _kernelRadius, [&](size_t i, const Point2D& neighborPosition) {
        double dist = origin.distanceTo(neighborPosition);
...
Vector2D SphSystemData2::interpolate(const Point2D& origin, const ConstArrayAccessor1<Vector2D>& values) const {
    Vector2D sum;
    auto d = densities();
    SphStdKernel2 kernel(_kernelRadius);
    const double m = mass();
...
Vector2D SphSystemData2::gradientAt(size_t i, const ConstArrayAccessor1<double>& values) const {
    ...
    const Point2D& origin = p[i];
    SphSpikyKernel2 kernel(_kernelRadius);
    const double m = mass();

    for (size_t j : neighbors) {
        const Point2D& neighborPosition = p[j];
        double dist = origin.distanceTo(neighborPosition);
        if (dist > 0.0) {
...
void SphSystemData2::computeMass() {
    ...
    BoundingBox2D sampleBound(Point2D(-1.5 * _kernelRadius, -1.5 * _kernelRadius),
                              Point2D(1.5 * _kernelRadius, 1.5 * _kernelRadius));
