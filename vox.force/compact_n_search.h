...
public:
     ...
     * @param x Pointer to the point position data. Must point to continuous data of 3 * n
     * real values.
    ...
    /**
     * Performs the actual query for a single point. This method returns a list of neighboring points. Note: That
     * points_changed() must be called each time when the positions of a point set changed.
     */
    ...
    /**
     * Performs the actual query for a single point x. This method returns a list of neighboring points in all existing
     * point sets. Note: That points_changed() must be called each time when the positions of a point set changed.
     */
     ...
     * Otherwise, in case of using point-wise search (find_neighbors(i, j, neighbors)) the method must be called
     * explicitly.
     ...
     * to invoke the sort_field function on the position array. Moreover, be aware the grid has
     * been reinitialized after each sort. Therefore, the points should not be reordered too
     * frequently.
     */
    void z_sort();
    ...
    /** Activate/Deactivate all point set pairs containing the given index. If search_neighbors is true, neighbors in
     * all other point sets are searched. If find_neighbors is true, the new point set is activated in the neighborhood
     * search of all other point sets.
     *   @param i Index of searching point set.
     *   @param search_neighbors If true/false enables/disables that point set i searches points in all other point
     * sets.
    ...
    /** Returns true if point set i search points in point set j.
