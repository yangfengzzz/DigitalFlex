...
void PhysicsAnimation::advanceTimeStep(double timeIntervalInSeconds) {
    ...
    if (_isUsingFixedSubTimeSteps) {
        LOGI("Using fixed sub-timesteps: {}", _numberOfFixedSubTimeSteps)
        ...
        for (unsigned int i = 0; i < _numberOfFixedSubTimeSteps; ++i) {
            LOGI("Begin onAdvanceTimeStep: {}  (1/{}) seconds", actualTimeInterval, 1.0 / actualTimeInterval)

            utility::Timer timer;
            onAdvanceTimeStep(actualTimeInterval);

            LOGI("End onAdvanceTimeStep (took {} seconds)", timer.Elapsed())

            _currentTime += actualTimeInterval;
        }
    } else {
        LOGI("Using adaptive sub-timesteps")
        ...
        while (remainingTime > kEpsilonD) {
            ...
            LOGI("Number of remaining sub-timesteps: {}", numSteps)
